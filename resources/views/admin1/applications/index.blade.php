    ...
    <th>updated at</th>
                            ...
                            @foreach($applications as $row)
                                <tr>
                                    <td>{!!  $row->id !!}</td>
    <td>{!!  $row->name !!}</td>
    <td>{!!  $row->aws_account_id !!}</td>
    <td>{!!  $row->repository !!}</td>
    <td>{!!  $row->deployment_type !!}</td>
    <td>{!!  $row->url !!}</td>
    <td>{!!  $row->sonar_key !!}</td>
    <td>{!!  $row->json_rules !!}</td>
    <td>{!!  $row->created_at !!}</td>
    <td>{!!  $row->updated_at !!}</td>
    
                                    <td>   
                                        {!! Form::open(['route' => ['applications.delete',$row->id], 'method' => 'delete']) !!}
                                        <div class='btn-group'>
                                            <a href="{!! route('applications.show', [$row->id]) !!}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-eye-open"></i></a>
                                            <a href="{!! route('applications.edit', [$row->id]) !!}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-edit"></i></a>
