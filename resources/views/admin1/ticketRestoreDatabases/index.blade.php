...
@section('content')
   <section id="widget-grid" class="">
                        ...
                        <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                            <thead>			                
                                <tr>
                                 <th>id</th>
    <th>ticket id</th>
    <th>production server id</th>
    <th>production database</th>
    ...
    <th>log</th>
                            ...
                            @foreach($ticketRestoreDatabases as $row)
                                <tr>
                                    <td>{!!  $row->id !!}</td>
    <td>{!!  $row->ticket_id !!}</td>
    <td>{!!  $row->production_server_id !!}</td>
    <td>{!!  $row->production_database !!}</td>
    ...
    <td>{!!  $row->log !!}</td>
                                    ...
                                    <td>   
                                        {!! Form::open(['route' => ['ticketrestoredatabases.delete',$row->id], 'method' => 'delete']) !!}
                                        <div class='btn-group'>
                                            <a href="{!! route('ticketrestoredatabases.show', [$row->id]) !!}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-eye-open"></i></a>
                                            <a href="{!! route('ticketrestoredatabases.edit', [$row->id]) !!}" class='btn btn-default btn-xs'><i class="glyphicon glyphicon-edit"></i></a>
